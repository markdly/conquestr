...
Now we can regenerate our syntax this time using our custom changes:

```{r syntax_custom}
foo_syntax <- cqc_syntax("foo", "1-9", cmds = foo_cmds, lookup_vals = foo_lookup)
writeLines(unlist(foo_syntax)) 
```
...
```{r}
writeLines(unlist(foo_syntax), here::here("foo.cqc")) 
```
